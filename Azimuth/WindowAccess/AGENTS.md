...
| `WindowFrameWriter.swift` | `@MainActor`. AX position/size 쓰기. 권한·settable 가드, 애니메이션 억제(대상 앱의 `AXEnhancedUserInterface`/`AXManualAccessibility` 끄기 — VoiceOver 게이트·PID별 지연 복원), shrink일 때 size→position 순서, 제약 앱은 실제 크기로 anchored origin을 1회만 쓰기 + verify·1회 재시도, 적용 후 실제 frame 회신 |

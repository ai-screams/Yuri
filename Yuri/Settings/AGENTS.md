...
## Purpose
설정창의 윈도우 컨트롤러. 창 생성·표시·멀티모니터 배치를 담당하고, 의존성을 내용 뷰(`ViewController`, `Yuri/`에 위치)에 주입한다.
...
- 설정 UI의 **내용**은 `Yuri/ViewController.swift`에 있다(이 컨트롤러는 창 프레이밍·표시만). 새 의존성은 이 init을 통해 `ViewController`로 전달.
...
### Internal
- `Yuri/ViewController`(내용), `Preferences/PreferencesStore`, `Launch/LaunchAtLoginService`.
